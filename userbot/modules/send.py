...
DELETE_TIMEOUT = 5
thumb_image_path = "https://telegra.ph/file/993518265ca9e73252945.jpg"
...
@bot.on(admin_cmd(pattern="instal"))
async def instal(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern=r"unlod (?P<shortname>\w+)$"))
async def unlod(event):
    if event.fwd_from:
        return
...
@bot.on(admin_cmd(pattern=r"lod (?P<shortname>\w+)$"))
async def lod(event):
    if event.fwd_from:
        return
    shortname = event.pattern_match["shortname"]
