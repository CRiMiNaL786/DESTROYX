...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        # support for other third-party modules
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[Here](https://t.me/Javessupport)"
            ...
            ftext += "--------BEGIN Javes USERBOT TRACEBACK LOG--------"
            ftext += "\nDate: " + date
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        spec.loader.exec_module(mod)
        sys.modules[
            "userbot.modules.assistant" + "Initialising Javes" + shortname
        ] = mod
        sedprint.info("Javes Has imported " + shortname)
...
def load_assistant(shortname):
    if shortname.startswith("__"):
        ...
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        print("Initialising Javes.")
        print("Javes - Imported " + shortname)
    else:
        import importlib
        import sys
        from pathlib import Path
        ...
        print("Javes Has imported " + shortname)
